...
def creates_images_by_class(generator_model, emotion_label):
    ...
    noise_images, classes = create_latent_batch_vectors(batch_size, latent_vector_size, class_size, device,probs=[1/7,1/7,1/7,1/7,1/7,1/7,1/7])
    if emotion_label!=-1:
...
def main():
    ...
    gen.to(device)
    classifier=torch.load("classifier.txt").to(device)
    classifier.eval()
